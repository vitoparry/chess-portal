    ...
    const fetchCSVData = async () => {
      ...
      for (const source of DATA_SOURCES) {
        try {
          ...
          const roundsParsed = Papa.parse(roundsText, { header: true, skipEmptyLines: true, transformHeader: (h) => h.trim().toLowerCase() });
          ...
          const completedCount = roundsParsed.data.filter((r: any) => {
            ...
            const w = parseFloat(r[wKey] || '0');
            const b = parseFloat(r[bKey] || '0');
            return (w + b) > 0;
          }).length;
          ...
          newLeaders[source.category] = topPlayers.map(p => ({ ...p, rank: uniqueScores.indexOf(p.points) + 1 }));
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-4 relative overflow-x-hidden">
      <style jsx global>{`
        ...
        .animate-vertical-scroll { animation: vertical-scroll 45s linear infinite; }
      ...
      <div className="w-full max-w-[1400px] grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 relative z-10">
        ...
        <div className="md:col-span-5 flex flex-col justify-center gap-6 w-full order-2">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <Link href="/standings" className="group"><button className="w-full py-4 bg-amber-600 hover:bg-amber-500 rounded-xl font-bold text-lg shadow-lg transition transform group-hover:-translate-y-1">🏆 STANDINGS</button></Link>
                    <Link href="/groups" className="group"><button className="w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold text-lg shadow-lg transition transform group-hover:-translate-y-1">⚔️ ROUNDS</button></Link>
                </div>
                <div className="grid grid-cols-2 gap-3">
                    <Link href="/rules" className="group"><button className="w-full py-3 bg-slate-800 hover:bg-slate-700 rounded-xl font-medium text-sm border border-slate-600 text-slate-300 hover:text-white transition">📜 Rules</button></Link>
                    <Link href="/procedure" className="group"><button className="w-full py-3 bg-slate-800 hover:bg-slate-700 rounded-xl font-medium text-sm border border-slate-600 text-slate-300 hover:text-white transition">♟️ Guide</button></Link>
                </div>
                <Link href="/archive" className="group"><button className="w-full py-3 bg-slate-900 hover:bg-slate-800 rounded-xl font-bold text-sm border border-slate-700 text-slate-500 hover:text-slate-300 transition uppercase tracking-widest shadow-lg">📂 Match Archive</button></Link>
            </div>
        </div>
        ...
        <div className="md:col-span-4 flex flex-col gap-5 h-full order-3 w-full">
            <div className="bg-slate-900/50 rounded-2xl p-5 border border-slate-800 shadow-xl backdrop-blur-sm h-full flex flex-col">
                <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2 border-b border-slate-800 pb-2"><span>👑</span> Top Leaders</h2>
                    ...
                    {['Adults', 'Juniors', 'Kids'].map((cat) => {
                        ...
                        return (
                            <div key={cat} className="mb-4">
                                <h3 className={`text-[10px] font-bold uppercase tracking-widest mb-2 ${cat==='Adults'?'text-amber-500':cat==='Juniors'?'text-blue-500':'text-green-500'}`}>{cat}</h3>
            ...
            <div className="bg-slate-900/50 rounded-2xl p-5 border border-slate-800 shadow-xl backdrop-blur-sm">
                <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2 border-b border-slate-800 pb-2"><span>📊</span> Matches Played</h2>
