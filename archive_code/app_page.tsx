          ...
          const roundsParsed = Papa.parse(roundsText, { 
            ...
            transformHeader: (h) => h.trim().toLowerCase()
              ...
              const completedCount = roundsParsed.data.filter((r: any) => {
                ...
                const wKey = keys.find(
                  k => k.includes('white') && (k.includes('point') || k.includes('pts') || k.includes('score'))
                );
                const bKey = keys.find(
                  k => k.includes('black') && (k.includes('point') || k.includes('pts') || k.includes('score'))
                );

                if (!wKey || !bKey) return false;

                let wStr = r[wKey]?.toString().trim();
                let bStr = r[bKey]?.toString().trim();

                // Normalize half-points
                if (wStr === '½') wStr = '0.5';
                if (bStr === '½') bStr = '0.5';
                ...
                if (isNaN(w) || isNaN(b)) return false;

                // Valid completed game totals
                const total = w + b;
