...
// ✅ Robust "concluded game" detection
...
const isConcludedRow = (r: any): boolean => {
  ...
  const resultKey =
    ...
    keys.find((k) => k.toLowerCase().includes('score') && !k.toLowerCase().includes('white') && !k.toLowerCase().includes('black'));
  ...
  // 2) Try to find explicit White/Black score columns (more forgiving than before)
  const wKey =
    keys.find((k) => k.toLowerCase().includes('white') && (k.toLowerCase().includes('point') || k.toLowerCase().includes('pts') || k.toLowerCase().includes('score'))) ||
    keys.find((k) => k.toLowerCase() === 'w' || k.toLowerCase() === 'white_pts' || k.toLowerCase() === 'white_points');

  const bKey =
    keys.find((k) => k.toLowerCase().includes('black') && (k.toLowerCase().includes('point') || k.toLowerCase().includes('pts') || k.toLowerCase().includes('score'))) ||
    keys.find((k) => k.toLowerCase() === 'b' || k.toLowerCase() === 'black_pts' || k.toLowerCase() === 'black_points');
  ...
  // 3) Fallback: scan row for exactly two score-like values (0 / 0.5 / 1), ignoring meta columns
  const candidates: number[] = [];
  for (const k of keys) {
    if (looksLikeMetaColumn(k)) continue;

    // also ignore obvious player-name columns
  ...
  // If at least two candidates exist, take the first two and validate (most sheets have only two score cells)
  if (candidates.length >= 2) {
    // Try all pairs (safer if there are extra numeric columns that slipped through)
